...
public class Imputacion_Por_Variables_Ficticias {
	...
	public static void main(String[] args) {
		...
		List<Double> l_pregnancies = fictitiousVariable(r1);
		...
		List<Double> l_glucose = fictitiousVariable(r2);
		...
		List<Double> l_bloodPressure = fictitiousVariable(r3);
		...
		List<Double> l_skinThickness = fictitiousVariable(r4);
		...
		List<Double> l_insulin = fictitiousVariable(r5);
		...
		List<Double> l_BMI = fictitiousVariable(r6);
		...
		List<Double> l_DiabetesPedigreeFunction = fictitiousVariable(r7);
		...
		List<Double> l_Age = fictitiousVariable(r8);
...
System.out.println("\n");
	...
	public static List<Double> fictitiousVariable(List<Double> data) {
        List<Double> imputedData = new ArrayList<>();
        double sum = 0;
        int count = 0;

        //Calculamos la media de la lista de datos sin los valores nulos
        ...
        //Generamos una variable ficticia, indicando si el valor es nulo o no
        for (Double value : data) {
            if (value != null) {
                imputedData.add(value);
            } else {
                imputedData.add(mean);
            }
        }
        return imputedData;
    }
